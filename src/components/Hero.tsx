...
const Hero: React.FC = () => {
  ...
  const handleEmailClick = (e: React.MouseEvent) => {
    e.preventDefault();
    const mailtoLink =
      'mailto:jayanthutukuri.dev@gmail.com' +
      '?subject=' +
...
[your name]`
    ...
    window.open(mailtoLink, '_self'); // _self ensures external mail client opens even under SPA routing
  };

  // Rotating roles
  const roleVariants = ['Software Developer', 'Software Engineer', 'Java Developer'];
      ...
      <div className="container mx-auto px-6 text-center relative z-10 pointer-events-auto">
        <motion.div initial={{ opacity: 0, y: 50 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
          ...
          >
            <div className="w-full h-full rounded-full bg-gradient-to-r from-blue-500 via-purple-500 to-pink-500 p-1">
              <div className="w-full h-full rounded-full bg-slate-800 flex items-center justify-center relative overflow-hidden">
                <motion.div whileHover={{ scale: 1.05 }} className="w-full h-full rounded-full overflow-hidden relative">
                  <img
                    src={profilePic}
          ...
          <div className="flex justify-center space-x-6 mb-6">
            ...
            {/* LinkedIn */}
            <motion.a
              href="https://www.linkedin.com/in/utukurijayanth/"
              target="_blank"
              rel="noopener noreferrer"
              whileHover={{ scale: 1.2, y: -5 }}
              whileTap={{ scale: 0.9 }}
